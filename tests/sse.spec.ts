/**
 * openai-completions protocol contract only: the [DONE] sentinel and
 * STREAM_CLOSED on EOF without it. SSE framing (chunk splits, CRLF,
 * multi-data joins, comments) is eventsource-parser's contract, not
 * re-proven here beyond the comment callback.
 */
...
function bytes(...fragments: string[]): ReadableStream<Uint8Array> {
...
/** Drain one async iterable to completion, past any sentinel. */
...
describe('parseSse', () => {
  it('yields event payloads and stops at the DONE sentinel', async () => {
